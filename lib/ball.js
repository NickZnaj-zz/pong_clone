...
Ball.prototype.move = function(timeDelta) {
  ...
  if (this.pos[1] <= 0 + this.radius + fudgeFactor) {
    this.vel[1] = -(this.vel[1]);
    this.changedDir = true;
  }
  if (this.pos[1] >= this.canvasSize[1] - this.radius - fudgeFactor) {
    this.vel[1] = -(this.vel[1]);
    this.changedDir = true;
  }
  if (this.pos[0] <= 0 + this.radius + fudgeFactor) {
    this.vel[0] = -(this.vel[0]);
    this.changedDir = true;
  }
  if (this.pos[0] >= this.canvasSize[0] - this.radius - fudgeFactor) {
    this.vel[0] = -(this.vel[0]);
    this.changedDir = true;
  }
};
